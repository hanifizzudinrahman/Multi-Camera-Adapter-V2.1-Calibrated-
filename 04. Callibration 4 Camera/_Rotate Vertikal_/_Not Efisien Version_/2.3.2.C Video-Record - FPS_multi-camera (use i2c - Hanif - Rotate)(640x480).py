...
def displayImage(img, mode, FPS):
	...
	if not len(img) <= 0 and mode == 1:
		...
		# ========================================= Rotate Vertikal
		imgCropA = cv2.rotate(imgCropA, cv2.ROTATE_90_CLOCKWISE)
		imgCropB = cv2.rotate(imgCropB, cv2.ROTATE_90_CLOCKWISE)
		imgCropC = cv2.rotate(imgCropC, cv2.ROTATE_90_CLOCKWISE)
		imgCropD = cv2.rotate(imgCropD, cv2.ROTATE_90_CLOCKWISE)
		...
		# Pixel Ditukar (Because Vertikal)
		image = np.zeros((2 * pixelXCrop, 2 * pixelYCrop, 3), dtype=np.uint8)
		image[0:pixelXCrop, 0:pixelYCrop, :] = imgCropA   # 0 = Camera A
		image[pixelXCrop:, 0:pixelYCrop, :] = imgCropC    # 2 = Camera C
		image[0:pixelXCrop, pixelYCrop:, :] = imgCropB    # 1 = Camera B
		image[pixelXCrop:, pixelYCrop:, :] = imgCropD    # 3 = Camera D
...
if __name__ == "__main__":
	...
	with picamera.PiCamera() as camera:
		...
		# Dimensi Ditukar (Because Vertikal)
		out = cv2.VideoWriter(VideoPathSave, fourcc, 20.0, (2*pixelYCrop, 2*pixelXCrop))
