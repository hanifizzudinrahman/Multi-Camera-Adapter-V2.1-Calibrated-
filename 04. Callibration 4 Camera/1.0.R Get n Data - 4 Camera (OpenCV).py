    ...
    if cap.isOpened():
        ...
        while cv2.getWindowProperty("CSI Camera " + str(cam), 0) >= 0:
            ...
            if key == ord("q"):
                ...
                trigger = 1
                if cam == 1:
                    cv2.imwrite("Hasil_Gambar/R.Camera_A_ %s.jpg" % now, img)
                    break
            elif key == ord("d"):
                done = 1
                trigger = 0
            ...
            if trigger == 1 and cam != 1:
                if FinalTime >= 2:
                    if cam == 2:
                        cv2.imwrite("Hasil_Gambar/R.Camera_B_ %s.jpg" % now, img)
                        break
                    elif cam == 3:
                        cv2.imwrite("Hasil_Gambar/R.Camera_C_ %s.jpg" % now, img)
                        break
                    elif cam == 4:
                        cv2.imwrite("Hasil_Gambar/R.Camera_D_ %s.jpg" % now, img)
                        break
        cap.release()
